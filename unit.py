...
class Unit(Block):
                    ...
                    if len(mapping) > 0:
                        ...
                        if self.task is not None:
                            ...
                            for item in self.inventory:
                                item.update(self.x * config.image_size[0], self.y * config.image_size[1])
                                to_drop.append(item)

                            if self.viewport.item_layer[self.y][self.x] != None:
                                self.viewport.item_layer[self.y][self.x].extend(to_drop)
                            else:
                                self.viewport.item_layer[self.y][self.x] = to_drop
    ...
    def do_task(self):
        ...
        elif isinstance(self.task, IdlingTask):
            self.work_target = None
            self.path = None
            self.task.do()
    ...
    def path_to(self, pos):
        ...
        if not self.path:
            ...
            if len(self.path) > 1:
                ...
                self.rect.x = self.path[1][0] * config.image_size[0]

                self.y = self.path[1][1]
                self.rect.y = self.path[1][1] * config.image_size[1]
